...
public class BindLdapHandler extends BaseLdapHandler {
	...
	public List<Response> messageReceived(Request request, IoSession ioSession,
		...
		String saslMechanism =
			GetterUtil.getString(bindRequest.getSaslMechanism());

		Response response = null;

		if (saslMechanism.equals(DIGEST_MD5)) {
			response =
				getSaslResponse(bindRequest, ioSession, ldapHandlerContext);
		}
		else if (bindRequest.isSimple()) {
			response =
				getSimpleResponse(bindRequest, ioSession, ldapHandlerContext);
		}
		else {
	...
	protected Response getSaslResponse(
		...
		if (saslServer.isComplete()) {
			...
			return getResponse(
				bindRequest, ResultCodeEnum.SUCCESS);
		}
		else {
			return getResponse(
	...
	protected String getValue(Dn dn, String normType) {
		for (Rdn rdn : dn) {
			...
			String rdnNormValue = rdn.getNormValue().getString();
