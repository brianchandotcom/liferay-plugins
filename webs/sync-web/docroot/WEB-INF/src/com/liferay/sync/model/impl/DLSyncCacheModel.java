...
public class DLSyncCacheModel implements CacheModel<DLSync>, Externalizable {
	...
	public DLSync toEntityModel() {
		...
		if (checksum == null) {
			dlSyncImpl.setChecksum(StringPool.BLANK);
		}
		else {
			dlSyncImpl.setChecksum(checksum);
		}
		...
		dlSyncImpl.setLockUserId(lockUserId);

		if (lockUserName == null) {
			dlSyncImpl.setLockUserName(StringPool.BLANK);
		}
		else {
			dlSyncImpl.setLockUserName(lockUserName);
		}

		dlSyncImpl.setSize(size);
